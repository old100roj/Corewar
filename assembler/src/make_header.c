...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/31 14:39:03 by iganich           #+#    #+#             */
/*   Updated: 2019/01/31 14:39:05 by iganich          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	char name_or_comment)
{
	...
	if (res == 1)
		(ft_strlen(temp) < PROG_NAME_LENGTH) ? ft_strcpy(header->prog_name,
			temp) : ft_handle_errors(make_error(*l, 0, 24, NULL), NULL);
	else
		(ft_strlen(temp) < COMMENT_LENGTH) ? ft_strcpy(header->comment,
			temp) : ft_handle_errors(make_error(*l, 0, 25, NULL), NULL);
	ft_strdel(&temp);
	return (res);
}
